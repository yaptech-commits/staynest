...
export async function createOrUpdateLocalUser(params: {
  ...
  const openId =
    existing?.openId ??
    `local_${Math.random().toString(36).substring(2, 12)}_${Date.now()}`;
  const role =
    params.role ??
...
export async function listApprovedHotels() {
  ...
  return db
    .select()
    .from(hotels)
    .where(eq(hotels.approvalStatus, "approved"))
...
export function calculateRoomAvailability(
  room: { id: number; totalRooms: number },
  bookingRows: Array<{
    roomId: number;
    checkInDate: string;
    checkOutDate: string;
  ...
  const bookedRooms = bookingRows.filter(
    booking =>
      ...
      ["booked", "checked_in"].includes(booking.bookingStatus) &&
      rangesOverlap(
        booking.checkInDate,
  ...
  const blocked = blockedRows.some(
    block =>
      (block.roomId === null || block.roomId === room.id) &&
      rangesOverlap(block.startDate, block.endDate, checkInDate, checkOutDate)
  );
...
export async function listRoomAvailabilityForHotel(input: {
  hotelId: number;
  checkInDate: string;
  checkOutDate: string;
}) {
...
export function buildManualHotelValues(input: {
  ownerId: number;
  name: string;
  location: string;
  address?: string;
...
export function isHotelOwner(
  hotel: { ownerId: number } | undefined,
  ownerId: number
) {
  return Boolean(hotel && hotel.ownerId === ownerId);
}

export async function createHotelForOwner(
  input: Parameters<typeof buildManualHotelValues>[0]
) {
  const db = await getDb();
...
export async function updateHotelForOwner(input: {
  id: number;
  ownerId: number;
  values: Partial<typeof hotels.$inferInsert>;
}) {
  const db = await getDb();
  if (!db) return false;
  const result = await db
    .update(hotels)
    .set(input.values)
    .where(and(eq(hotels.id, input.id), eq(hotels.ownerId, input.ownerId)));
  return result[0].affectedRows > 0;
}
...
export async function updateBookingStatusForHotel(input: {
  id: number;
  hotelId: number;
  bookingStatus:
    | "booked"
...
export function buildManualRoomValues(input: {
  hotelId: number;
  name: string;
  roomType: string;
...
export async function updateRoomForHotel(input: {
  id: number;
  hotelId: number;
  ...
  const result = await db
    .update(rooms)
    .set(input.values)
    .where(and(eq(rooms.id, input.id), eq(rooms.hotelId, input.hotelId)));
  return result[0].affectedRows > 0;
}
...
export async function deleteBlockedDateForHotel(id: number, hotelId: number) {
  ...
  const result = await db
    .delete(blockedDates)
    .where(and(eq(blockedDates.id, id), eq(blockedDates.hotelId, hotelId)));
  return result[0].affectedRows > 0;
}
...
export async function recordAvailabilityEvent(
  input: import("../drizzle/schema").InsertAvailabilityEvent
) {
  const db = await getDb();
  if (!db)
    return {
      id: 0,
      ...input,
    } as import("../drizzle/schema").AvailabilityEvent & { id: number };
  const result = await db
    .insert((await import("../drizzle/schema")).availabilityEvents)
    .values(input);
  return { id: Number(result[0].insertId), ...input };
}

export async function updateBookingFromBillFlow(input: {
  bookingReference: string;
  bookingStatus: "booked" | "cancelled" | "conflict_flagged";
  conflictDetails?: string | null;
}) {
  const db = await getDb();
  if (!db) return false;
...
export async function recordBookingFinance(input: {
  bookingId: number;
  hotelId: number;
  currency: string;
  grossAmount: number;
  ...
  const ledger = await db
    .insert((await import("../drizzle/schema")).commissionLedger)
    .values({
  ...
  const payout = await db
    .insert((await import("../drizzle/schema")).payouts)
    .values({
      hotelId: input.hotelId,
      currency: input.currency,
...
export async function listPayouts() {
  ...
  return db
    .select()
    .from((await import("../drizzle/schema")).payouts)
    .orderBy(desc((await import("../drizzle/schema")).payouts.createdAt));
}

export function buildOnboardingPersistencePayload(input: {
  userId: number;
  role: "guest" | "partner";
  fullName: string;
  email: string;
  ...
  return {
    user: {
      ...
      email: input.email,
      role:
        input.role === "partner" ? ("hotel_owner" as const) : ("user" as const),
    },
    profile: {
      userId: input.userId,
      role: input.role,
...
type OnboardingProfilePersistenceStore = {
  updateUser: (
    userId: number,
    values: { name: string; email: string; role: "user" | "hotel_owner" }
  ) => Promise<void>;
...
export async function saveOnboardingProfile(
  ...
  injectedStore?: OnboardingProfilePersistenceStore
) {
  ...
  const result = await db
    ...
    .where(
      and(
        ...
        gt(onboardingProfiles.emailVerificationExpiresAt, new Date())
      )
    );
  return result[0].affectedRows > 0;
}

export async function notifyAdminsOfPartnerApplication(input: {
  userId: number;
...
export async function getPayoutAccountForHotel(hotelId: number) {
  ...
  const res = await db
    .select()
    .from(partnerPayoutAccounts)
    .where(eq(partnerPayoutAccounts.hotelId, hotelId))
    .limit(1);
  return res[0] ?? null;
}
