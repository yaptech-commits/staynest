...
describe("StayNest commercial rules", () => {
  it("calculates the fixed 15% platform commission", () => {
    expect(calculateCommission(1000)).toEqual({ commission: 150, hotelPayout: 850 });
    expect(calculateCommission(2850)).toEqual({ commission: 427.5, hotelPayout: 2422.5 });
  });
  ...
  it("returns an explicit availability fallback when BillFlow credentials are absent", async () => {
    ...
    try {
      await expect(getLiveAvailability({ roomTypeId: "101", checkInDate: "2026-09-01", checkOutDate: "2026-09-03" })).resolves.toEqual({ source: "demo", availableRooms: null, livePricing: null });
    } finally {
  ...
  it("creates a reservation contract without hiding the source when BillFlow is not configured", async () => {
    ...
    delete process.env.BILLFLOW_API_KEY;
    try {
      const result = await createBillFlowReservation({ propertyId: "property-1", roomTypeId: "Garden King", checkInDate: "2026-09-01", checkOutDate: "2026-09-03" });
